...
- `conda` with `conda-libmamba-solver` takes 10-12mins, also failed 3 of 10 times:
  ...
  - subsequent `conda env update` call prints this before failing ([GHA link](https://github.com/runsascoded/libmamba-solver-test/actions/runs/4969272579/jobs/8892339145#step:3:499)):
    ...
    ```
- The `conda-libmamba-solver` solve appears to be correctly using `libmamba`; most of the elapsed time is spent outputting blocks like this, every 10s ([example][combined run conda-libmamba-solver job 1]):
    ...
    The plain `conda` (`classic` solver) jobs don't reference `libmamba` ([example][combined run conda job 1]).
